...
const Salesman: React.FC = () => {
  ...
  const renderStepContent = () => {
    ...
    switch (currentStep) {
        ...
        return (
          <div className="space-y-5">
            <div className="rounded-md border border-gray-200 bg-white p-5">
              ...
              <h3 className="mt-2 text-base font-semibold text-gray-900">第三者责任险</h3>
              <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
                {[10, 20, 50, 100, 150, 200].map((amount) => (
                  <motion.button
                    ...
                    className={cn(
                      "rounded-md border py-3 text-center text-sm font-medium transition-all",
            ...
            <div className="rounded-md border border-gray-200 bg-white p-5">
              <h3 className="text-base font-semibold text-gray-900">车上人员责任险 (驾驶员)</h3>
              <div className="mt-3 grid grid-cols-3 gap-3 sm:grid-cols-4">
      ...
      default:
        return (
          <div className="rounded-md border border-dashed border-slate-200 bg-white p-5 text-center text-sm text-gray-600">
            {currentStep} 步骤内容
          </div>
        );
    }
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-100">
      <header className="flex items-center justify-between bg-slate-950 px-4 py-4 text-white">
        <button className="text-xl font-bold" type="button">
          ×
        </button>
      ...
      <div
        className={cn(
          ...
          isNev ? "bg-emerald-500" : "bg-emerald-600"
        )}
      >
        <button className="text-xl" type="button">
          ←
        </button>
        <h2 className="text-lg font-semibold">报价信息填写</h2>
        <button className="text-xl" type="button">
          ◎
        </button>
      </div>
      ...
      {status && (
        <div className="mx-4 mt-4 rounded-md border border-slate-200 bg-white p-4">
            ...
            {status !== "COMPLETED" && qrImage && (
              <>
                <img
                  src={qrImage}
                  alt="投保二维码"
                  className="h-40 w-40 rounded-md border border-slate-100 bg-white p-2"
                />
              </>
            )}
            {status !== "COMPLETED" && !qrImage && (
      ...
      <footer className="fixed bottom-0 left-0 right-0 border-t border-gray-200 bg-white p-4">
        <button
          ...
          className={cn(
            "w-full rounded-md py-4 text-lg font-semibold text-white transition-all",
            isButtonDisabled()
